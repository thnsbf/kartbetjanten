...
uniform float coneAngle; // half-angle in radians
uniform float view_angle_degrees;
...
{ 
    ...
    shadowParameters.depthBias *= max(log(depth + 1.0) * 0.001, 1.0);
   ...
   float nDotL = clamp(dot(vec3(1.0), -directionEC), 0.0, 1.0); 
    ...
    float visibility = czm_shadowDepthCompare(shadowMap, shadowParameters.texCoords, shadowParameters.depth);
    ...
    if (shadowPosition.z < 0.001 || shadowPosition.z > 1.0) {
        FragColor = color;
        return;
    }
